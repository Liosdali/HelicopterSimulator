...
namespace Oyedoyin.RotaryWing
{
    public class SilantroRotor : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            if (rotorConfiguration != RotaryController.RotorConfiguration.Conventional) { m_controller.m_torqueMode = RotaryController.TorqueMode.Corrected; }
            if (rotorConfiguration == RotaryController.RotorConfiguration.Coaxial && coaxialPosition == CoaxialPosition.Bottom) { δv = 0.5616f; δQ = 1; }

            // ------------------------------------------- Tandem Config
            if (rotorConfiguration == RotaryController.RotorConfiguration.Tandem)
            {
        ...
        private void AnalyseForces(double dt)
        {
            ...
            // Tandem Lateral Yaw
            if (rotorConfiguration == RotaryController.RotorConfiguration.Tandem && m_forcePoint != null)
            {
            ...
            if (groundEffect == GroundEffectState.Consider && rotorType == RotorType.MainRotor)
            {
                ...
                float zR = AGL / bladeRadius; if (zR > 3f) { zR = 3f; }
            ...
            float δY = 1;
            if (m_controller.m_torqueMode == RotaryController.TorqueMode.Corrected) { δY = (float)Math.Abs(m_controller.m_computer.m_yaw); }

            // --------------------------------- Rotate Model
            if (double.IsNaN(Ω) || double.IsInfinity(Ω)) { Ω = 0.0f; }
            if (m_hinge != null) { Handler.Rotate((Ω * 9.548), dt, m_hinge, rotorDirection, rotationAxis); }
            ...
            if (model == SimulationModel.NonUniformInflow)
            {
                for (int i = 0; i < subdivision + 1; i++)
                {
                    ...
                    FComplex αr = θr - ϕr;
                    double υz = Math.Sqrt(uw * uw + ww * ww);
                    double υTip = (Ωm * ri) + υz;
                    double soundSpeed = m_controller.m_core.m_atmosphere.a;
                    double υM = υTip / soundSpeed;
                    vMach[i] = υM;
                    ...
                    {
                        double rootCL = m_rootAirfoil.liftCurve.Evaluate((float)αs.m_real);
                        double tipCL = m_tipAirfoil.liftCurve.Evaluate((float)αs.m_real);
                        double rootCD = m_rootAirfoil.dragCurve.Evaluate((float)αs.m_real);
                        double tipCD = m_tipAirfoil.dragCurve.Evaluate((float)αs.m_real);
                        CL = MathBase.EstimateSection(rootCL, tipCL, xi);
                        CD = MathBase.EstimateSection(rootCD, tipCD, xi);
                    }
                    else
                    {
                        double rootCL = m_rootSuperfoil.GetCL(υM, αs.m_real);
                        double tipCL = m_tipSuperfoil.GetCL(υM, αs.m_real);

                        double rootCD = m_rootSuperfoil.GetCD(υM, αs.m_real);
                        double tipCD = m_tipSuperfoil.GetCD(υM, αs.m_real);
            ...
            // --------------------------------- Moment
            double LBs = 0.5f * e * Nb * Ω * Ω * -Iβ * β1s; // Check if proper inertia value is being used i.e Iβ or Mw
            double MBs = 0.5f * e * Nb * Ω * Ω * -Iβ * β1c;
            double Ls = LBs - Q * Math.Sin(β1c) * Math.Cos(β1s);
            double Ms = MBs + Q * Math.Cos(β1c) * Math.Sin(β1s);
            ...
            {
                if (rotorConfiguration == RotaryController.RotorConfiguration.Tandem) { m_direction = m_forcePoint.up; }
                else { m_direction = transform.up; }

                Vector3 xforce = m_force.x * transform.right;
                Vector3 yforce = m_force.y * m_direction;
                Vector3 zforce = m_force.z * transform.forward;
                m_controller.force += xforce;
                m_controller.force += yforce;
                m_controller.force += zforce;
                m_controller.moment += m_moment;

                m_controller.m_rigidbody.AddForceAtPosition(xforce, transform.position, ForceMode.Force);
                m_controller.m_rigidbody.AddForceAtPosition(yforce, transform.position, ForceMode.Force);
                m_controller.m_rigidbody.AddForceAtPosition(zforce, transform.position, ForceMode.Force);
                if (m_controller.m_torqueMode == RotaryController.TorqueMode.Corrected) { m_moment.y = 0; }
                m_controller.m_rigidbody.AddRelativeTorque(m_moment, ForceMode.Force);
            }
            else
            {
                Vector3 tailForce = m_force.y * δY * transform.up;
                m_controller.m_rigidbody.AddForceAtPosition(tailForce, transform.position, ForceMode.Force);
            }
        }
        /// <summary>
        /// 
        ...
        {
            float m_alpha = (float)(Ω / Ωmax);
            if (visualType == VisulType.Complete)
            {
                if (blurredRotor != null && normalRotor != null)
                {
                    foreach (Material brotor in blurredRotor) { if (brotor != null) { brotor.color = new Color(blurredRotorColor.r, blurredRotorColor.g, blurredRotorColor.b, m_alpha); } }
                    foreach (Material nrotor in normalRotor) { if (nrotor != null) { nrotor.color = new Color(normalRotorColor.r, normalRotorColor.g, normalRotorColor.b, (1 - m_alpha) + normalBalance); } }
                }
            }
