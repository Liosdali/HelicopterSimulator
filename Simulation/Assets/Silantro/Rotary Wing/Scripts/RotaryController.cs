    ...
    public class RotaryController : Controller
    {
        ...
        protected override void Initialize()
        {
            ...
            if (allOk)
            {
                ...
                if (m_torqueMode == TorqueMode.Corrected) { m_rigidbody.angularDrag = m_core.rotationDrag; } else { m_rigidbody.angularDrag = 0.02f; }
        ...
        protected override void ComputeFixedUpdate()
        {
            ...
            if (allOk && isInitialized)
            {
                ...
                // Rotors
                if (m_rotors != null) { foreach (SilantroRotor rotor in m_rotors) { rotor.Compute(_fixedTimestep); _torqueSum += (float)rotor.Torque; } }
                // Turboshaft
                ...
                if (isControllable)
                {
                    ...
                    if (m_configuration != RotorConfiguration.Conventional) { m_rigidbody.AddRelativeTorque(m_moment, ForceMode.Force); }
                }
            }
        }
        /// <summary>
            ...
            if(allOk)
            {
                ...
                { 
                    foreach (SilantroTurboshaft shaft in m_shafts) 
                    {
                        ...
                        Debug.Log("Updating throttle input  || _throttleInput = " + _throttleInput);
                        Debug.Log("Updating throttle input  || gameobj = " + gameObject.name);
                        Debug.Log("Updating throttle input  || shaft.throttle = " + shaft.throttle);
                    } 
                }
            }
        }
