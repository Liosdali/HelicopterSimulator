...
namespace Oyedoyin.RotaryWing
{
    ...
    public class SilantroStabilizer : MonoBehaviour
    {
        ...
        private void AnalyseForces()
        {
            ...
            Vector3 liftDirection = Vector3.Cross(m_controller.m_rigidbody.velocity, transform.right).normalized;
            Vector3 lift = liftDirection * (float)m_lift;
            m_controller.force += lift;
            m_controller.m_rigidbody.AddForceAtPosition(lift, transform.position, ForceMode.Force);

            // Drag is always opposite of the velocity.
            Vector3 drag = -m_controller.m_rigidbody.velocity.normalized * (float)m_drag;
            m_controller.force += drag;
            m_controller.m_rigidbody.AddForceAtPosition(drag, transform.position, ForceMode.Force);
        }
    }
    #endregion
